...
const EntropyRule = require(path.resolve(__dirname, 'rules', 'entropy-rule'));
const FuzzyMatchRule = require(path.resolve(__dirname, 'rules', 'fuzzy-match-rule'));
...
Shield.prototype.loadConfigFile = function loadConfigFile(filePath) {
  if(!filePath.endsWith('.json')){
    filePath = path.resolve(__dirname, '..', 'config', 'rules', filePath + '.json');
  }
  ...
  return new Promise((resolve, reject) => {
    fs.readFile(filePath, (err, data) => {
      ...
      for(const ruleName in config.regex) {
        ...
        this.rules.push(this.loadRegexRule(ruleName, config.regex[ruleName]));
      }

      for(const ruleName in config.entropy) {
        if(!config.entropy.hasOwnProperty(ruleName)) continue;
        this.rules.push(new EntropyRule(ruleName, config.entropy[ruleName]));
      }

      for(const ruleName in config.fuzzy) {
        if(!config.fuzzy.hasOwnProperty(ruleName)) continue;
        this.rules.push(new FuzzyMatchRule(ruleName, config.fuzzy[ruleName]));
      }
...
Shield.prototype.loadRegexRule = function loadRegexRule(ruleName, config) {
  config.pattern = config.pattern.replace(/\\{2,}/, '\\');
  return new RegexRule(ruleName, config);
};
